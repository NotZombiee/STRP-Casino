            ...
            if IsControlJustReleased(0, 38) then
            	ESX.TriggerServerCallback("okokBanking:GetPIN", function(pin)
            		if pin then
						...
						ESX.ShowAdvancedNotification("Bank", "Head up to a bank to set a PIN code", "fas fa-university", "red", 3)
					end
				end)
            end
	...
	elseif data.action == "deposit" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				...
				ESX.ShowAdvancedNotification("Bank", "Invalid amount", "fas fa-exclamation-triangle", "red", 3)
			end
		else
			ESX.ShowAdvancedNotification("Bank", "Invalid Input", "fas fa-exclamation-triangle", "red", 3)
		end
			...
			if tonumber(data.value) > 0 then
				ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
					...
					elseif isUsed == nil then
						ESX.ShowAdvancedNotification("Bank", "This IBAN does not exist", "fas fa-university", "red", 3)
					end
				end, data.iban:upper())
			else
				ESX.ShowAdvancedNotification("Bank", "Invalid amount", "fas fa-exclamation-triangle", "red", 3)
	...
	elseif data.action == "change_iban" then
		if Config.CustomIBANAllowLetters then
			...
			ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				elseif isUsed ~= nil then
					ESX.ShowAdvancedNotification("Bank", "This IBAN is already in use", "fas fa-exclamation-triangle", "red", 3)
				end
			end, iban)
		elseif not Config.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				ESX.ShowAdvancedNotification("Bank", "You can only use numbers on your IBAN", "red", 3)
			end
		end
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				...
				ESX.ShowAdvancedNotification("Bank", "Your PIN needs to be 4 digits long", "fas fa-exclamation-triangle", "red", 3)
			end
		else
			ESX.ShowAdvancedNotification("Bank", "You can only use numbers", "fas fa-exclamation-triangle", "red", 3)
		end
	end
end)
